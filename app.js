...
const App = {
    ...
    sections: [
        ...
        },
        {
            ...
            icon: '✅',
            description: 'Patients who need insurance verification',
            type: 'patient-select',
    ...
    cacheElements() {
        this.elements = {
            ...
            insVerifyPatientGrid: document.getElementById('insVerifyPatientGrid'),
            insVerifyItemsList: document.getElementById('insVerifyItemsList'),
            insVerifyCountBadge: document.getElementById('insVerifyCountBadge'),
            ...
            exportToDriveBtn: document.getElementById('exportToDriveBtn'),
    ...
    bindEvents() {
        ...
        // Completion Screen
        if (this.elements.exportToDriveBtn) {
            this.elements.exportToDriveBtn.addEventListener('click', () => this.exportAllToGoogleDrive());
        }
    ...
    // Also handles simple name format: "FirstName LastName" (one per line)
    ...
    parsePatientSchedule(text) {
        ...
        // Helper function to capitalize a name word
        ...
        const extractName = (text) => {
            // Remove appointment types from the end (case insensitive)
            let remainder = text;
            ...
            for (const word of words) {
                ...
                // Accept both uppercase and lowercase (will capitalize later)
                if (/^[A-Za-z][A-Za-z'-]*$/.test(word)) {
                    nameWords.push(capitalizeName(word));
        ...
        for (const line of lines) {
            ...
            // First, try to match the time pattern (schedule format)
            const match = trimmedLine.match(timePattern);
            if (match) {
                nameWords = extractName(match[2].trim());
            } else {
                // No time prefix - try parsing as a simple name
                nameWords = extractName(trimmedLine);
            }
    ...
    async updateChiro180Status(patient, status) {
        ...
        if (this.huddleData.chiro180[patient] === status) {
            this.huddleData.chiro180[patient] = 'pending';
        } else {
    ...
    renderInsuranceVerifyList() {
        ...
        if (!this.huddleData.insuranceVerify || !Array.isArray(this.huddleData.insuranceVerify)) {
            this.huddleData.insuranceVerify = [];
        }

        // Render patient grid
        const addedPatients = new Set(this.huddleData.insuranceVerify.map(i => i.patientName));

        this.elements.insVerifyPatientGrid.innerHTML = this.patients.map(p => {
            const isAdded = addedPatients.has(p);
            return `
                <button class="patient-chip ${isAdded ? 'added' : ''}" data-patient="${this.escapeHtml(p)}">
                    ${this.escapeHtml(p)}
                    ${isAdded ? ' ✓' : ''}
                </button>
            `;
        ...
        // Add click handlers
        this.elements.insVerifyPatientGrid.querySelectorAll('.patient-chip').forEach(chip => {
            chip.addEventListener('click', () => {
                const patient = chip.dataset.patient;
                this.toggleInsuranceVerifyPatient(patient);
            });
        });

        // Render items list
        this.renderInsuranceVerifyItems();
    },

    renderInsuranceVerifyItems() {
        const items = this.huddleData.insuranceVerify || [];
        this.elements.insVerifyCountBadge.textContent = items.length;

        if (items.length === 0) {
            this.elements.insVerifyItemsList.innerHTML = '<p class="empty-state">No patients added yet. Tap a patient below to add.</p>';
            return;
        }

        this.elements.insVerifyItemsList.innerHTML = items.map(item => `
            <div class="added-item" data-id="${item.id}">
                <div class="item-content">
                    <span class="item-patient">${this.escapeHtml(item.patientName)}</span>
                    <span class="item-note">Needs insurance verification</span>
                </div>
        ...
        // Add remove handlers
        this.elements.insVerifyItemsList.querySelectorAll('.remove-item-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                this.removeInsuranceVerifyItem(btn.dataset.id);
            });
        });
    },

    async toggleInsuranceVerifyPatient(patient) {
        if (!this.huddleData.insuranceVerify) {
            this.huddleData.insuranceVerify = [];
        }

        const existingIndex = this.huddleData.insuranceVerify.findIndex(i => i.patientName === patient);

        if (existingIndex >= 0) {
            // Remove if already added
            this.huddleData.insuranceVerify.splice(existingIndex, 1);
        } else {
            // Add new item
            this.huddleData.insuranceVerify.push({
                id: Date.now().toString(),
                patientName: patient,
                date: StorageManager.getTodayString(),
                timestamp: new Date().toISOString()
    ...
    async removeInsuranceVerifyItem(id) {
        this.huddleData.insuranceVerify = this.huddleData.insuranceVerify.filter(i => i.id !== id);
        await this.saveData();
        this.renderInsuranceVerifyList();
    },
    ...
    showCompletionScreen() {
        ...
        this.sections.forEach(section => {
            ...
            if (section.id === 'todo24') {
                ...
                count = (this.huddleData.insuranceVerify || []).length;
            } else {
    ...
    async exportCurrentSection() {
        ...
        if (section.id === 'todo24') {
            ...
            // Already an array
            items = this.huddleData.insuranceVerify || [];
        } else {
    ...
    // Export all sections to Google Drive
    async exportAllToGoogleDrive() {
        // Check if Google Drive is connected
        if (!GoogleDriveManager.isConnected()) {
            this.showToast('Please connect to Google Drive in Settings first', 'error');
            return;
        }

        this.showToast('Exporting to Google Sheets...', 'success');

        const sections = ['pmtIssues', 'insuranceQuestions', 'noAppt', 'chargePassdown', 'todo24', 'chiro180', 'insuranceVerify'];
        let exportedCount = 0;
        ...
        for (const sectionId of sections) {
            ...
            if (sectionId === 'todo24') {
                ...
                items = this.huddleData[sectionId] || [];
            }
            ...
            try {
                await ExcelExporter.exportSectionToDrive(sectionId, items);
                exportedCount++;
            } catch (e) {
                console.error(`Failed to export ${sectionId}:`, e);
            }
        }

        if (exportedCount > 0) {
            this.showToast(`Exported ${exportedCount} sections to Google Sheets`, 'success');
        } else {
